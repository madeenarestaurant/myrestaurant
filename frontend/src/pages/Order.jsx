                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                        <div className="space-y-3">
                            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                                <span className="text-[#8C231F] font-black text-[9px] uppercase tracking-[0.4em] mb-1.5 block">Premium Dining</span>
                        ...
                        <div className="w-full md:w-auto">
                            <div className="relative group">
                                <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-[#8C231F] transition-colors" size={16} />
                                <input 
                                    type="text"
                                    placeholder="Search delicacy..."
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    className="bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 w-full md:w-64 text-[13px] outline-none focus:border-[#8C231F]/50 focus:bg-white/10 transition-all"
                                />
                            </div>
                        </div>
                    </div>
            ...
            <div className="max-w-[1300px] mx-auto px-6 pb-40">
                {loading ? (
                    <div className="h-64 flex flex-col items-center justify-center gap-3">
                        ...
                        <p className="text-[9px] font-black uppercase tracking-widest text-gray-600">Syncing Delicacies...</p>
                    </div>
                ) : (
                ...
                {cartCount > 0 && (
                    ...
                    >
                        <div className="max-w-3xl mx-auto">
                            <div className="bg-[#1a1a1a]/95 backdrop-blur-2xl rounded-[2rem] border border-white/10 shadow-[0_-15px_40px_rgba(0,0,0,0.4)] overflow-hidden">
                                <div className="flex flex-col md:flex-row items-center justify-between p-3 md:p-4 gap-4 md:gap-6">
                                            ...
                                            {Object.values(cart).map(item => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex-1 min-w-[50px]">
                                                        <p className="text-[9px] font-bold line-clamp-1">{item.name}</p>
                                                        <p className="text-[8px] text-[#8C231F] font-black">×{item.quantity}</p>
                                                    </div>
                                    ...
                                    <div className="flex items-center gap-5 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 md:border-l border-white/5 pt-3 md:pt-0 md:pl-6">
                                        <div className="text-left md:text-right">
                                            <p className="text-[8px] font-black text-gray-500 uppercase tracking-widest leading-none mb-1">Subtotal</p>
                                            <p className="text-xl font-serif text-white tracking-widest leading-none">OMR {Number(cartTotal).toLocaleString()}</p>
                                        </div>

                                        <button 
                                            onClick={() => navigate('/cart')}
                                            className="bg-[#8C231F] hover:bg-white hover:text-[#8C231F] text-white px-6 py-4 rounded-xl flex items-center gap-2.5 transition-all duration-500 group shadow-lg shadow-[#8C231F]/20 active:scale-95"
                                        >
                                            <span className="text-[10px] font-black uppercase tracking-[0.2em]">Go to Cart</span>
...
const CategoryTab = ({ label, active, onClick }) => (
    <button 
        onClick={onClick}
        className={clsx(
            "px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-[0.3em] transition-all whitespace-nowrap border",
            active 
    ...
    >        {/* Extra Compact Image Section */}
        ...
        <div className="p-4 pt-2.5">
            <div className="flex items-start justify-between gap-2 mb-1 min-h-[2.4rem]">
                <h3 className="text-[13px] font-serif tracking-wide text-white leading-tight group-hover:text-[#8C231F] transition-colors line-clamp-2 flex-1">{product.name}</h3>
                <span className="text-[10px] font-black text-[#8C231F] whitespace-nowrap mt-0.5">OMR {product.price}</span>
            </div>
            <p className="text-[8px] text-gray-600 font-medium leading-relaxed mb-4 line-clamp-2 h-6">{product.description}</p>
            ...
            <div className="flex items-center justify-between gap-2.5">
                {quantity > 0 ? (
                    ...
                    <button 
                        onClick={onAdd}
                        className="flex-1 py-2 bg-[#8C231F]/10 hover:bg-[#8C231F] text-[#8C231F] hover:text-white border border-[#8C231F]/20 rounded-lg font-black text-[8px] uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-1.5 group/btn active:scale-95"
                    >
